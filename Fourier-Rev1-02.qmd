...
subtitle:
  revised-version  $\newline$
  non-peer reviewed version $\newline$
  *confidential version, do not distribute*
lang: en
date: today
author:
...
The classic cinematic effect of wheels or propellers appearing to stop and start to rotate backwards as they speed up is an expression of the **Sampling criterion** discussed below in [@sec-expe].
The star-like diffraction patterns that you see when looking at distant lights at night is due to the errors in the
discretization of the signal by the light sensitive cells on the retina of your eye, which is effectively a biological digitization of the signal.
...
$$s(t) = \sum_k^K \; A_k e^{i\phi_k} \; e^{(-\frac 1 {\tau_k} + i 2\pi f_k ) t} $$ {#eq-12cpx}
The time independent term $A_k e^{i\phi_k}$ is the complex amplitude of the component, while the complex value $e^{(-\frac 1 {\tau_k} + i 2\pi f_k )}$ which acts on the time, fully caracterizes the time evolution.
...
the *Sampling criterion* states that
...
$$ SW = \frac{1}{2 \Delta t} \qquad \text{or} \qquad \Delta t = \frac{1}{2 SW}$$  {#eq-Sampling}
@eq-Sampling is the sampling theorem, and the rate $\Delta t$ is called the Nyquist sampling rate.
This important result is found in the classical work from C.E.Shannon [@Shannon_1948], but was previously discovered by several other authors, and is also known as the Shannon theorem or Nyquist-Shannon theorem.
...
It is equivalent in many points to the analytical FT and shares most of its characteristics, with the notable exception of the Sampling theorem presented above, a property of the sampling operation, not of the FT itself.
...
If we acquire $N$ real points with a sampling $\Delta t$ given by @eq-Sampling, we have a total acquisition time of $T_{max} = N \times \Delta t$.
Similarly, we have for the spectral width $SW = F_{max} = N/2 \times \Delta f$.
...
Combining these two expressions with @eq-Sampling, it comes:
...
This relation is the equivalent of the Sampling theorem for the frequency spectrum.
It is not a coincidence if this expression, in the form
$\Delta f \times  T_{max} = 1$
is reminiscent with the Heisenberg uncertainty principle, and is sometimes called the Fourier uncertainty principle.[@Hogan2005]
...
The Sampling theorem ([@eq-Sampling]), is a direct result of the discretization of the real signal data.
...
It turns out, the sampling theorem defines that to most-accurately record a time-varying signal, the digitization time-step must be at least $2 \times$ faster than the event.
...
In any case, it is always advised to use a high quality low-pass filter to remove signals above the highest expected frequency, usually half the sampling frequency, to avoid spurious signals and noise folding back into the spectrum.
...
Provided the sampling criterion is met, then after the FT the signal will be composed of peaks as above in Figures 3-5.
...
Thus, given the chosen data acquisition rate (keeping the Sampling limit, above in mind), the total number of data points required is readily calculated – if the operator knows the value of $\tau$.
...
and so, in quadratic detection, the Sampling criterion is modified to:
...
FT is heavily used in all forms of analytical methods which rely on any image creation or image analysis: microscopy of any kind (visible, fluorescence, Raman, Electronic, etc.),
Magnetic Resonance Imaging (MRI), or even X-ray diffraction.

In image analysis, the direct domain is the physical space $(x,y)$ or $(x,y,z)$ (in Å, µm or Mpc !) and the reciprocal space is related to spacial frequencies in the image, noted here $(\xi, \upsilon, \zeta)$.
The definition of FT in @eq-ftdef and @eq-ftm1def has to be extended to 2 or 3 dimensions as follows (here in 3D):

$$ \text{FT}_{3D}[s(x,y,z)] = S(\xi, \upsilon, \zeta) =  \iiint_{-\infty}^{+\infty} s(x,y,z) e^{-2i \pi (\xi x + \upsilon y + \zeta z)} dx dy dz$$ {#eq-2DFT}

$$ \text{FT}_{3D}^{-1}[S(\xi, \upsilon, \zeta)] = s(x,y,z) =  \iiint_{-\infty}^{+\infty} S(\xi, \upsilon, \zeta) e^{2i \pi (\xi x + \upsilon y+ \zeta z)} d\xi d\upsilon d\zeta$$

All the properties and theorems presented in [@sec-expe] and [@sec-results] still hold but have to be interpreted with care, as the direct and reciprocal domains, being multidimentionnal, are defined differently.
...
Somehow, the X-ray beam diffracting in the crystal, or the magnetic field gradient dispersing the spin resonances, perform an analogic FT of the studied object, and a simple 3D FT restores the studied volume.
...
Note that the FT used in multidimensional processing is not equivalent to the FT used in image or diffraction analysis, but similar to two sequential 1D FT.
It uses slightly different mathematics which allow to compute a different phase for each frequency axis.[@Ernst87; @Delsuc_1988; @Orekhov_2023]
...
How can we separate frequencies closer to each other than the minimum separation defined by the Sampling theorem?
...
Based on assumptions different from the central assumptions of the classical Fourier theory, it even allows, in certain conditions, to overcome the Sampling theorem in the time domain (sub-Nyquist sampling) and the frequency domain (super-resolution)
...
**Sampling criterion**
: (or Nyquist criterion) determines the highest frequency faithfully sampled by a given periodic sampling.
...
**Shift theorem:** There is a relationship between the position in one domain and the phase in the reciprocal domain.
From the property of the exponentiation: $e^{a+b} = e^a e^b$ and the definition of FT (@eq-ftdef)
it is possible to show the two related expressions for $s(t)$ and $S(f)$ related by FT:

- $s(t-t_0)$ is transformed into $e^{-2i\pi f t_0}S(f) \qquad$ <br>
and
- $e^{i 2 \pi f_0 t}s(t)$ is transformed into $S(f - f_0)$.

in other words, a time shift $t_0$ in the signal creates in the spectrum a phase shift proportional to the frequency,
and multiplying the signal with a constant frequency $f_0$ shifts the spectrum by this frequency.
...
The code used to generate the figures can be found at [https://github.com/delsuc/Fourier-Transform-Review](https://github.com/delsuc/Fourier-Transform-Review)
